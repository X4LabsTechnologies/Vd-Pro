...
```http
GET /api/v1/search?q=اسم%20الفيلم&site=example.com&category=movies_series&extract=true&quality=auto&deep=true
Authorization: Bearer YOUR_TOKEN
```

المعامل الاختياري `site` يقيّد البحث بنطاق الموقع الذي تكتبه، مثل `example.com` أو اسم نطاق كامل. والمعامل الاختياري `category` يختار مجموعة المصادر من `config/sources.json` مثل `movies_series` أو `anime` أو `music` أو `general_video`. عند وجود فئة، يبدأ المسار بمصادر الكتالوج بتوازٍ محدود وببحث خفيف لكل مصدر ضمن `CATALOG_SEARCH_MS`، يحتفظ بالنتائج المتحققة والبدائل، ثم ينتقل إلى البحث العام عند عدم كفايتها أو انتهاء ميزانية الكتالوج. يستخدم البحث الآن مخططًا تكيفيًا: يبدأ باكتشاف سريع، ثم يوسع إلى نية المشاهدة، ثم يستخدم مصادر البيانات الوصفية عند الحاجة فقط، ويتوقف عن التصعيد عند العثور على مرشح مشاهدة قوي. يرسل المحرك استعلامات مشاهدة عامة، ثم يحتفظ فقط بالنتائج التي تطابق النطاق ويصنفها إلى `watchCandidates` و`infoCandidates`. عند استخدام `extract=true` يحاول استخراج الوسائط من أفضل مرشح مشاهدة مطابق، ولا يستخرج من صفحات المعلومات. عند غياب مرشح مشاهدة مطابق يعيد `NO_WATCH_CANDIDATE`. يبقى إرسال `url` مباشرة إلى `/api/v1/extract` مدعومًا كما هو.

### ملف المصادر

يحتوي `config/sources.json` على بيانات المصادر التي يديرها المستخدم. أضف المصدر داخل الفئة المناسبة بصيغة `name` و`url` و`enabled` و`priority`. لا تضف منطقًا خاصًا بالموقع؛ يستخدم كل مصدر نفس البحث والاستخراج والتحقق. المواقع التي لا تملك رابطًا كاملًا لا تُدرج حتى يكتمل رابطها.
